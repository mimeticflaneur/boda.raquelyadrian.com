...
${ADMIN_FONTS}
<style>${ADMIN_CSS}
.login{max-width:400px;margin:20vh auto 0;padding:2.4rem 2.2rem;background:var(--paper);border:1px solid var(--ink);position:relative}
...
.login p{font-size:.72rem;color:var(--mute);text-align:center;margin:.4rem 0 1.6rem;text-transform:uppercase;letter-spacing:.18em}
...
<body><form class="login" method="GET" action="/admin">
<h1>Raquel <em>&amp;</em> Adrián</h1><p>Panel de la boda</p>
<input name="token" type="password" placeholder="Token de acceso" autofocus>
...
<header class="top">
  ...
  <nav class="tools">
    <a class="accent" href="/export/personas.csv?${q}">CSV por persona</a>
    <a href="/export/rsvp.csv?${q}">CSV respuestas</a>
    <a href="/export/canciones.csv?${q}">CSV canciones</a>
  </nav>
</header>

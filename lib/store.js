...
// Limite de envios por IP. Evita que un bot —o un bucle de reintentos— llene
// la base de datos de basura. Contador con INCR + EXPIRE: la primera visita
// crea la clave y le pone caducidad; a partir de ahi solo incrementa.
// Si el contador falla por lo que sea, NO se bloquea a nadie: preferimos un
// duplicado a perder una confirmacion de verdad.
async function overLimit(kind, ip, max, windowSec) {
  if (!ip) return false;
  try {
    const key = `boda:rl:${kind}:${ip}`;
    const n = Number(await redis(['INCR', key]));
    if (n === 1) await redis(['EXPIRE', key, String(windowSec)]);
    return n > max;
  } catch (e) {
    console.error('Rate limit no disponible:', e.message);
    return false;
  }
}
...
module.exports = { isConfigured, append, readAll, updateById, deleteById, overLimit };
